...
export default {
  ...
  computed: {
    $_ocTag_class() {
      const classes = ["oc-file-tag", `oc-file-tag-${getSizeClass(this.size)}`]

      this.type === "router-link" || this.type === "a"
        ? classes.push("oc-file-tag-link")
        : classes.push(`oc-file-tag-${this.type}`)
...
```js
<div>
<oc-file-tag size="small">
  <oc-icon name="links" size="small" />
  Small tag
</oc-file-tag>
<oc-file-tag size="medium">
  <oc-icon name="links" size="medium" />
  Medium tag
</oc-file-tag>
<oc-file-tag size="large">
...
<oc-grid gutter="small" flex="true">
    <oc-file-tag class="oc-mr-s">
      <oc-icon name="group" />
      Shared with other people
    </oc-file-tag>
    <oc-file-tag class="oc-mr-s" type="a">
      <oc-icon name="links" />
      Shared via link
    </oc-file-tag>
    <oc-file-tag class="oc-mr-s" type="button">Expires in 2 days</oc-file-tag>
</oc-grid>
```
</docs>
