<template> </template>
<script>
import OcImg from "../elements/OcImage"

/**
 * Shows how to layout and structure the logon screen.
 *
 * ## Todo:
 * - [ ] where to get the background image from? should be themable -> less/scss ??
 * - [ ] where to get the logo image from? should be themable -> less/scss ??
 * - [ ] define proper elements
 */
export default {
  name: "Login",
  components: { OcImg },
  status: "deprecated",
  release: "1.0.0",
}
</script>

<docs>
    ```
    <template>
        <div class="uk-cover-container uk-height-medium" style="height: 800px">
            <img src="https://raw.githubusercontent.com/owncloud/core/master/core/img/background.jpg" alt="" uk-cover>
            <div class="uk-section uk-flex uk-flex-middle uk-animation-fade" uk-height-viewport>
                <div class="uk-width-1-1">
                    <div class="uk-container">
                        <div class="uk-grid-margin uk-grid uk-grid-stack" uk-grid>
                            <div class="uk-width-1-1@m">
                                <div class="uk-card">
                                    <div class="uk-card-body">
                                        <oc-img class="uk-position-center" src="data:image/svg+xml;base64," />
                                    </div>
                                </div>
                                <div class="uk-margin uk-width-large uk-margin-auto uk-card uk-card-default uk-box-shadow-large">
                                    <div class="uk-card-body">
                                        <h3 class="uk-card-title uk-text-center">Welcome to ownCloud</h3>
                                        <form>
                                            <div class="uk-text-meta">
                                            Please click the button below to authenticate and get access to your data.
                                            </div>
                                            <div class="uk-margin">
                                                <button class="uk-button uk-button-primary uk-button-large uk-width-1-1">Authorize</button>
                                            </div>
                                        </form>
                                        <div class="uk-text-small uk-text-center">
                                            <a href="https://owncloud.org">ownCloud</a> – A safe home for all your data
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </template>
    ```
</docs>
